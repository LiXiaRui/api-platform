  ...
  <div class="packageBa">
    <el-container>
      <el-header style="border-bottom: 2px solid #d8dce6;">
        ...
        <span style="font-size:18px;font-weight:490;">API集市</span>
        ...
        <el-container>
            <el-header class="ex">
                ...
                <el-row :gutter="20">
                    ...
                    <el-col :span="2"><div>|</div></el-col>
                    <el-col :span="2"><div class="grid-content" @click="getAPI()">全部</div></el-col>
                    <el-col :span="2" v-for="kind in kinds" :key="kind.id"><div class="grid-content" @click="getAPIById(kind.id)">{{kind.name}}</div></el-col>
                </el-row>
            </el-header>
           <el-main>
...
export default {
  ...
  methods: {
      ...
      //获取所有API
      getAPI(){
        let url="/api/api-platform/v1/kong/routes"
      ...
      getAPIById(id){
        ...
        axios.get(url).then(response=>{
          if(response.status==200){
            console.log(response.data);
            this.items=response.data.kind.routes;
          }
        })
      }
    }
