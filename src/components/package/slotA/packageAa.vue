  ...
  <div class="packageBa">
    <el-container>
      <el-header style="border-bottom: 2px solid #d8dce6;">
        ...
        <span style="font-size:18px;font-weight:490;">数据商城</span>
        <span class="el-icon-arrow-right" style="font-size:18px;font-weight:490;">{{kindName}}</span>
      </el-header>
        <el-container>
            <el-header class="ex">
                <el-row :gutter="20">
                ...
                <div :data="items.slice((curPage-1)*pagesize,curPage*pagesize)" style="height:600px;">
                    <ul class="sortB">
                        <template v-for="(item) in items.slice((curPage-1)*pagesize,curPage*pagesize)">
                            <li :key="item.APIid" @mouseover="selectStyle (item)" 
                                :class="{'active':item.active}" 
                                @mouseout="outStyle(item)"
                                @click="goTo('detailB',item.id)">
                                <span class="APIimg"><img src="@/components/static/images/home/apilogo/1.jpg" width="100%"></span>
...
export default {
  ...
  methods: {
      ...
      getAll(){
        ...
        var url='/api/api-platform/v1/kind?id='+_this.id;
