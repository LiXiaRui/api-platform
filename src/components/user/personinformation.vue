          ...
          <el-tab-pane label="基本信息" name="first">
            <el-form :model="userlist" :rules="rules" ref="EditorUserForms">
                <el-form-item label="头像" prop="avatar_url" :label-width="formLabelWidth">
                    ...
                    <img v-if="userlist.avatar_url" :src="this.userlist.avatar_url" class="avatar">
...
/*****检验两次密码是否一致***/
  ...
  methods: {
    ...
    getUser() {
      ...
      axios.get(url,{
        ...
        if (response.status == 200) {
          ...
          this.userlist.phone=response.data.data.phone;
          this.userlist.idcard=response.data.data.idcard;
          this.loading = false;
    ...
    submitForm() {
      ...
      axios.put("/api/api-platform/v1/user",params).then(response => {
        if (response.status == 200) {
          this.$message({
            message: "保存成功",
            type: "success"
